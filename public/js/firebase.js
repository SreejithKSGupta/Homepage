...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', function() {
        navigator.serviceWorker.register('http://127.0.0.1:5500/sw.js?v=4').then(function(registration) {
        // Registration was successful
        console.log('ServiceWorker registration successful with scope: from outside', registration.scope);
    }, function(err) {
        // registration failed :(
...
function seteventlisteners() {
    ...
    gid("ftrppup").addEventListener("click", function () { showhide("footer"); });
...
async function editsite(editbtn){
    ...
    console.log("flagg in index = " + flagg);
    if (flagg == 0) {
        var site=editbtn.id;
        var sites=JSON.parse(localStorage.getItem('sites'));
        for(var i=0;i<sites.length;i++){
